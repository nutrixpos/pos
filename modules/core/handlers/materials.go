...
func MaterialWaste(config config.Config, logger logger.ILogger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var is_consume bool
		is_consume, err = strconv.ParseBool(isConsumeStr)
		if err != nil {
			http.Error(w, "Invalid quantity", http.StatusBadRequest)
			return
		}

		order_idStr := r.URL.Query().Get("order_id")
		if order_idStr == "" {
			http.Error(w, "order_id query string is required", http.StatusBadRequest)
			return
		}

		reasonStr := r.URL.Query().Get("reason")
		if reasonStr == "" {
		...
		err = material_svc.Waste(entry_id_param, material_id_param, quantity, order_idStr, reasonStr, is_consume)
		if err != nil {
...
func MaterialInventoryReturn(config config.Config, logger logger.ILogger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		params := mux.Vars(r)
