...
func (c *Core) RegisterHttpHandlers(router *mux.Router, prefix string) {
	...
	router.Handle(prefix+"/api/orders/{order_id}/refund", middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.RefundOrder(c.Config, c.Logger, c.Settings), "admin", "cashier"))).Methods("POST", "OPTIONS")
	...
	router.Handle(prefix+"/api/products/{id}/increase", middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.IncreaseProduct(c.Config, c.Logger), "admin"))).Methods("POST", "OPTIONS")
