...
func (c *Core) RegisterHttpHandlers(router *mux.Router, prefix string) {
	...
	router.Handle(prefix+"/api/orders", middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.SubmitOrder(c.Config, c.Logger), "admin", "cashier"))).Methods("POST", "OPTIONS")
	...
	router.Handle(prefix+"/api/orders/{id}pay", middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.Payorder(c.Config, c.Logger), "admin", "cashier"))).Methods("POST", "OPTIONS")
