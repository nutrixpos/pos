...
func (c *Core) RegisterHttpHandlers(router *mux.Router, prefix string) {
	...
	router.Handle(prefix+"/api/materials/{id}/entries", middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.PushMaterialEntry(c.Config, c.Logger), "admin", "cashier"))).Methods("GET", "OPTIONS")
