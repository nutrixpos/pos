...
func (c *Core) RegisterHttpHandlers(router *mux.Router, prefix string) {
	...
	router.Handle(prefix+"/api/purchase-orders", core_middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.CreatePurchaseOrder(c.Config, c.Logger), "admin"))).Methods("POST", "OPTIONS")
	router.Handle(prefix+"/api/purchase-orders", core_middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.GetPurchaseOrders(c.Config, c.Logger), "admin", "cashier"))).Methods("GET", "OPTIONS")
	router.Handle(prefix+"/api/purchase-orders/{id}", core_middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.GetPurchaseOrder(c.Config, c.Logger), "admin", "cashier"))).Methods("GET", "OPTIONS")
	router.Handle(prefix+"/api/purchase-orders/{id}", core_middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.CancelPurchaseOrder(c.Config, c.Logger), "admin"))).Methods("DELETE", "OPTIONS")
	router.Handle(prefix+"/api/purchase-orders/{id}/receive", core_middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.ReceivePurchaseOrder(c.Config, c.Logger), "admin"))).Methods("POST", "OPTIONS")
	router.Handle(prefix+"/api/grns", core_middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.GetGRNs(c.Config, c.Logger), "admin", "cashier"))).Methods("GET", "OPTIONS")
	router.Handle(prefix+"/api/grns/{id}", core_middlewares.AllowCors(auth_svc.AllowAnyOfRoles(handlers.GetGRN(c.Config, c.Logger), "admin", "cashier"))).Methods("GET", "OPTIONS")
